...
export default function AdminComparisonEditor() {
  ...
  useEffect(() => { setReviewed(false) }, [e, aSlug, bSlug])
    ...
    <div className="p-8 max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => void save()} disabled={saving || generating || (e.status === "published" && !reviewed)}>
            {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}<span className="ml-1.5">Save</span>
          </Button>
          {e.status === "published" ? (
            <Button size="sm" variant="ghost" onClick={() => void save("draft")} disabled={saving || generating}>Unpublish</Button>
          ) : (
            <Button size="sm" onClick={() => void save("published")} disabled={saving || generating || !reviewed}>Publish</Button>
          )}
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="rounded-xl border border-[#9a7b4f]/30 bg-[#9a7b4f]/5 p-4">
            <div className="flex items-center gap-1.5 text-sm font-semibold text-[#9a7b4f] mb-3">
              <Sparkles className="h-4 w-4" /> Generate research draft
            </div>
            ...
            <div className="mt-3 flex items-center gap-2">
              <Button onClick={() => void generate()} disabled={generating || saving || e.status !== "draft" || !aSlug || !bSlug}>
                {generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
                <span className="ml-1.5">{generating ? "Generating…" : "Generate"}</span>
              </Button>
            </div>
            {generating && <p className="text-xs text-muted-foreground mt-2">Generating a private research draft.</p>}
            ...
            <p className="mt-2 text-[11px] text-muted-foreground">
              Catalog inputs are unverified. Source review is required.</p>
          </div>
          ...
          <section className="space-y-3">
            ...
            {(e.faq ?? []).map((item, index) => (
              <div key={index} className="space-y-2">
                <input aria-label={`FAQ ${index + 1} question`} className={field} value={item.q}
                  onChange={event => set("faq", (e.faq ?? []).map((value, i) => i === index ? { ...value, q: event.target.value } : value))} />
                <textarea aria-label={`FAQ ${index + 1} answer`} className={field} value={item.a}
                  onChange={event => set("faq", (e.faq ?? []).map((value, i) => i === index ? { ...value, a: event.target.value } : value))} />
              </div>
            ))}
            <label className="flex items-start gap-2 text-sm">
              <input type="checkbox" checked={reviewed} onChange={event => setReviewed(event.target.checked)} />
              I checked the body, FAQ and metadata against linked sources, including model options and purchase claims.
            </label>
          </section>
        </div>
