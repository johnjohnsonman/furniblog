  ...
  return <main className="store-locations">
    ...
    <div className="location-wrap">
      ...
      {!r.country ? <section aria-labelledby="countries-title"><h2 id="countries-title">Browse by country</h2><div className="location-grid">{r.groups.map(g => <Link className="location-tile" key={g.code} href={g.path}><h3>{g.name}</h3><p>{g.stores.length} stores · {g.cities.length} cities</p><span>Explore stores →</span></Link>)}</div></section> : <>
        ...
        {r.country.code === "SG" && <section className="location-advice" aria-labelledby="singapore-guides-title"><h2 id="singapore-guides-title">Singapore chair shopping guides</h2><div className="location-card-actions"><Link href="/stores/guides/best-ergonomic-chair-showrooms-singapore">Best ergonomic chair showrooms →</Link><Link href="/stores/guides/where-to-try-herman-miller-chairs-singapore">Where to try Herman Miller chairs →</Link><Link href="/stores/guides/where-to-try-office-chairs-singapore">Where to try office chairs →</Link></div></section>}
