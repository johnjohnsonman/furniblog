...
const researchPath = "content/showrooms/global-expansion-wave.json";
...
const brandByName = new Map(
  registry.catalog.brands.map((brand) => [brand.name.toLowerCase(), brand]),
);
...
for (const candidate of candidates) {
  ...
  const brand = candidate.brand
    ? brandByName.get(candidate.brand.toLowerCase())
    : null;
  const bookingRequired = candidate.appointment === "required";
