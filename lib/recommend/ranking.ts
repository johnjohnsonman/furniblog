...
const AWARDS: Record<ResultAwardId, Omit<ResultAward, "id">> = {
  ...
  "long-workday": {
    title: "Best for Long Workdays",
    reason: "Strong physical fit with the best supporting adjustment evidence.",
  },
  "verified-data": {
...
export function rankRecommendationResults(
  ...
  give(
    ...
    winner(
      pool,
      (r) => r.fitStatus === "good",
      (r) => r.fit.score + r.why.length * 2 + (r.fit.evidence.length >= 3 ? 3 : 0),
    ),
  )
  give(
    "verified-data",
